// pages/market.tsx
// Area-level DLD transaction trend charts.
...
const AACOFAS = ['Downtown Dubai','Dubai Marina','Jumeirah Village Circle','Business Bay','Palm Jumeirah','DIFC','Dubai Hills Estate']
const ROOMS_LIST = ['Studio','1BR','2BR','3BR']

function ChangeChip({ pct }: { pct: number }) {
  const up = pct >= 0
  return (<span style={{fontSize:11,fontWeight:500,padding:'2px 8px',borderRadius:3,background:up?'#E1F5EE':'#FCEBEB',color:up?'#0F6E56':'#A32D2D''border:up?'1px solid #9FE1CB':'1px solid #F7C1C1'}}>{up?'+':''}{pct.toFixed(1)}%</span>)
}
...
function TrendChart({ trends, area, rooms }: { trends: AreaTrend[]; area: string; rooms: string }) {
  ...
  useEffect(() => {
    ...
    const series = trends.filter(t => (area ? t.area===area : AACOFAS.includes(t.area)) && t.rooms===rooms).slice(0,6)
    ...
    const datasets = series.map((s, i) => ({ label: s.area, data: allMonths.map(m => { const pt = s.trend.find(p => p.month===m); return pt?pt.medianPsf:null }), borderColor: PALETTEA[i%PALETTE.length], borderWidth:2, pointRadius:3, tension:0.35, spanGaps:true }))
    if (chartRef.current) chartRef.current.destroy()
    const ctx = canvas.getContext('2d')!
    chartRef.current = new window.Chart(ctx, { type:'line', data:{ labels:allMonths, datasets }, options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ position:'bottom' } }, scales:{ y:{ ticks:{ callback:(v: unknown)=>`${Number(v).toLocaleString()}` } } } } })
  ...
  return (
    <>
      ...
      <header style={{padding:'1rem 1.5rem',borderBottom:'1px solid var(--line)',background:'var(--surf)',display:'flex',alignItems:'center',gap:'1rem'}}>
        <Link href="/" style={{fontFamily:'var(--serif)',fontSize:'1.25rem',fontWeight:300,color:'var(--ink)',textDecoration:'none'}}>Dubai <em style={{fontWeight:500}}>Deals</em></Link>
        <span style={{color:'var(--line2)'}}>›</span>
        <span style={{fontFamily:'var(--serif)',fontWeight:300,color:'var(--ink3)'}}>{locale==='ja'?'エリア成約':'Trends'}</span>
      </header>
      <div style={{background:'var(--surf)',padding:'.7rem 1.5rem',borderBottom:'1px solid var(--line)',display:'flex',gap:8,flexWrap:'wrap',alignItems:center'}}>
        <select value={filterArea} onChange={e=>setFilterArea(e.target.value)} style={fltSt}>
          ...
          {AACOFAS.map(a=><option key={a}>{a}</option>)}
        </select>
        <div style={{display:'flex',border:'1px solid var(--line2)',borderRadius:6,overflow:'hidden'}}>
          {ROOMS_LIST.map(r=>(<button key={r} onClick={()=>{setFilterRooms(r);setChartArea('')}} style={{padding:'5px 12px',fontSize:12,border:'none',cursor:'pointer',background:filterRooms===r?'var(--surf2)':'transparent',fontWeight:filterRooms===r?500:400}}>{r}</button>))}
        </div>
        <button onClick={()=>load()} style={{...fltSt,cursor:'pointer'}}>{tx.refresh}</button>
      </div>
      {!error && !loading && (
        <div style={{padding:'1.5rem',}}>
          <section style={{background:'var(--surf)',border:'1px solid var(--line)',borderRadius:10,padding:'1.25rem',marginBottom:'1.5rem'}}>
            <h2 style={{fontFamily:'ap�--serif)',fontWeight:300,fontSize:'1rem',marginBottom:'1rem'}}>{filterRooms} PSF - 12 months {chartArea&&`: ${chartArea}`}</h2>
            {chartReady && trends.length>0;&& <TrendChart trends={trends} area={chartArea} rooms={filterRooms}/>}
          </section>
          <div style={{display:'grid',gridTemplateColumns:'speat(auto-fill,minmax(260px,1fr))',gap:1,background:'var(--line)'}}>
            {displayed.map(t => (
              <div key={`${t.area}:${t.rooms}`} style={{background:'var(--surf)',padding:'1rem 1.1rem',cursor:'pointer'}} onClick={()=>setChartArea(t.area)}>
                ...
                <div style={{fontFamily:'var(--mono)',fontSize:'1.15rem',fontWeight:500,marginBottom:4}}>{t.current.toLocaleString()} psf</div>
      ...
      <footer style={{padding:'1rem''textAlign:'center',fontSize:11,color:'var(--ink3)'}}>
        <p>{tx.dldNote}</p><p {tx.disclaimer}</p>
      </footer>
    </>
  )
}
