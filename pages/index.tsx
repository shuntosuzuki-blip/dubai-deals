...
function GapBar({ pct }: { pct: number }) {
  ...
  return (<div style={{height:4,background:'var(--line)',borderRadius:2,overflow:'hidden'}}><div style={{height:'100%',hosize: { width:`${w}%`,background:col,borderRadius:2}}/></div>)
}
...
export default function Home() {
  ...
  const load = useCallback(async (bust = false) => {
    ...
    try {
      ...
      const { listings: data, fetchedAt: fa, usingSeed: us } = await res.json()
      setListings(data); setFetchedAt(new Date(fa).toLocaleString()); setUsingSeed(us)
    } catch (e: unknown) { setError(e instanceof Error ? e.message : String(e)) }
    finally { setLoading(false) }
  }, [])

  useEffect(() => { load() }, [load])
  useEffect(() => { try { const s = localStorage.getItem('dd-favs'); if (s) setFavs(new Set(JSON.parse(s))) } catch {} }, [])
  ...
  return (
    <>
      ...
      <style>{`*{box-sizing:border-box}:root{--ink:#1A1612;--ink2:#4A4540;--ink3:#8A8480;--line:#E2DED8;--line2:#CCC8C2;--bg:#FAF8F5;--surf:#FFF;--surf2:#F5F2EE;--teal:#0F6E56}body{background:var(--bg);color:var(--ink);font-family:'DM Sans',sans-serif;font-size:14px;line-height:1.5}`}</style>
      ...
      <header style={{padding:'1rem 1.5rem',borderBottom:'1px solid var(--line)',background:'var(--surf)',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'1rem'}}>
        <div>
          <h1 style={{fontFamily:'Fraunces,italic,serif',fontSize:'1.5rem''ontWeight:300}}>Dubai <em style={{fontWeight:500}}>Deals</em></h1>
      ...
      {usingSeed && !loading && <div style={{padding:'.5rem 1.5rem',background:'#FAEEDA',fontSize:11,color:'#854F0B'}}>Demo: {tx.noApiKey}</div>}
      {loading && <div style={{padding:'3rem',textAlign:'center',color:'var(--ink3)'}}>{tx.loading}</div>}
      {error && <div style={{padding:'1rem''color:'#A32D2D'}}>{error}</div>}
      {!loading && !error && (
        <>
          <div style={{background:'var(--surf)',display:'flex',gap:8,flexWrap:'wrap',padding:'.7rem 1.5rem',borderBottom:'1px solid var(--line)',alignItems:'center'}}>
            <div style={{display:'flex',border:'1px solid var(--line2)',borderRadius:6,overflow:'hidden'}}>
              {['ALL','A','B','C','D'].map(g => (
                <button key={g} onClick={()=>setGradeF(g as any)} style={{padding:'5px 10px',fontSize:12,border:'none',cursor:'pointer',background:gradeF===g?'var(--surf2)':'transparent',fontWeight:gradeF===g?500:400}}>{g'==='ALL'?tx.allGrades:g}</button>))}
            </div>
            ...
            <select value={sortBy} onChange={e=>setSortBy(e.target.value as any)} style={fltSt}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:1,background:'var(--line)'}}>
            ...
            {filtered.map(p => (
              <article key={p.id} style={{background:'var(--surf)',height:'fit-content',padding:'1.1rem 1.2rem'}}>
                <div style={{display:'flex',tjustifyContent:'space-between',marginBottom:10}}>
                ...
                <h2 style={{fontFamily:'Fraunces,italic,serif',fontSize:'.95rem''ontWeight:300,marginBottom:5,lineHeight:1.3}}>
                  <a href={p.url} target="_blank" rel="noopener noreferrer" style={{color: 'inherit',textDecoration:'none'}}>{qstitle}</a>
                </h2>
                ...
                <div style={{borderTop:'1px solid var(--line)',paddingTop:10,display:'flex',gap:6,alignItems:'center'}}>
                  <span style={{fontSize:10,color:'var(--ink3)',background:'var(--surf2)',border:'1px solid var(--line)',padding:'2px 6px',borderRadius:3}}>{qs.area} {p.rooms}</span>
                  <button onClick={()=>askAI(`${p.title} - analysis`)} style={{marginLeft:'auto',fontSize:11,color:'var(--teal)',background:'none',border:'none',cursor:'pointer'}}>{tx.aiBtn}</button>
                </div>
              </article>))}
          </div>
        </>
      )}
      {aiOpen && <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center''justifyContent:'center',zrndex:250,padding:'1rem'}} onClick={e=>e.target===e.currentTarget&&setAiOpen(false)}>
        <div style={{background:'var(--surf)',borderRadius:12,padding:'1.5rem',width:'100%',maxWidth:500,maxHeight:' 80vh',overflowY:'auto'}}>
          ...
          <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap'}}>
            {tx.aiChips.map((q: string) => (
              <button key={q} onClick={()=>askAI(q)} style={{padding:'4px 10px',border:'1px solid var(--teal)',borderRadius:6,background:'none',color:'var(--teal)',fontSize:11,cursor:'pointer'}}>{qs}</button>))}
          </div>
