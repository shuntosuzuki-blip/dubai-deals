...
export interface AreaMarket {
  area: string; rooms: string; medianPsf: number; avgPsf: number; txnCount: number;
  samples: { date: string; psf: number }[]
}
export function normaliseRooms(raw: string): string {
  const s = (raw ?? '').toLowerCase(); if (s.includes('studio')) return 'Studio'
  const m = s.match(/(\d+)/); if (m) return `${m[1]}BR`; return raw
}
export async function fetchMarketStats(monthsBack = 6): Promise<AreaMarket[]> {
  ...
  url.searchParams.set('per_page','2000'); url.searchParams.set('page','1')
  ...
  const groups = new Map(), sampleMap = new Map()
  for (const r of rows) {
    const area = Number(r.procedure_area), worth = Number(r.actual_worth)
    if (!area||!worth) continue
    const psf = worth/area, key = r.area_name_en+'::'+normaliseRooms(String(r.rooms_en||''))
  ...
  for (const [key,psfs] of groups) {
    ...
    const sorted = [...psfs].sort((a,b)=>a-b), mid = Math.floor(sorted.length/2)
    const median = sorted.length%2===0?(sorted[mid-1]+sorted[mid])/2:sorted[mid]
    const avg = sorted.reduce((s,v)=>s+v,0)/sorted.length
    const samples = (sampleMap.get(key)||[]).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,6)
    stats.push({area,rooms,medianPsf:Math.round(median),avgPsf:Math.round(avg),txnCount:psfs.length,samples})
  }
  return stats
}
